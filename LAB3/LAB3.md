...
Мета: змінити номер телефону клієнта Іваненко Марії.

Очікуваний результат: у таблиці Customer буде оновлено номер телефону клієнта.
```sql
-- Оновлюємо номер телефону нового клієнта
UPDATE Customer
SET phone = '+380681111111'
WHERE email = 'maria.ivanenko@tour.com';

-- Перевіряємо оновлення даних клієнта
...
Мета: змінити ціну та кількість місць у новому турі до Праги.

Очікуваний результат: у таблиці Tour буде оновлено вартість туру та його місткість.

```sql
-- Оновлюємо ціну та місткість нового туру
UPDATE Tour
SET base_price = 17900.00,
    capacity = 18
WHERE title = 'Осінній тур до Праги';

-- Перевіряємо оновлення туру
...
Мета: змінити ціну додаткової послуги “Оренда туристичного спорядження”.

Очікуваний результат: у таблиці Service буде оновлено вартість додаткової послуги.

```sql
-- Оновлюємо ціну додаткової послуги
UPDATE Service
SET price = 1350.00
WHERE name = 'Оренда туристичного спорядження';

-- Перевіряємо оновлення послуги
SELECT name, price, description
FROM Service
WHERE name = 'Оренда туристичного спорядження';
```
